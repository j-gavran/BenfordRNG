...
def savefig(file_name, save_format="pdf", fig=None, data=None, tex_ancor=None, tight_layout=True, **save_kwargs):
    ...
    if fig or tex_ancor:
        ...
        pickle.dump({"data": data, "tex_ancor": [tex_ancor, save_format], "fig": fig}, open(save_p_location, "wb"))


def loadfig(file_name, set_save_location=True, load_dict=False):
    """Loads saved plot pickle."""

    if set_save_location:
        save_location = get_save_location(file_name)
    else:
        save_location = file_name

    p = pickle.load(open(save_location, "rb"))

    if type(p) == dict:
        if load_dict:
            return p

        fig, data, tex_ancor = p["fig"], p["data"], p["tex_ancor"]

        if data is None and tex_ancor is None:
            return fig, fig.axes
        elif tex_ancor is None:
            return fig, fig.axes, data
        elif data is None:
            return fig, fig.axes, tex_ancor
        else:
            return fig, fig.axes, data, tex_ancor
...
def display(fig, f=2):
    """Shows larger figure for notebook.
